...
public class WindowDoor extends Block
{
    public WindowDoor(final int par1, final Material par2Material)
    {
        ...
        setUnlocalizedName("windowdoor");
        func_111022_d(Archive.MOD_ID + ":window");
        setHardness(1);
        setResistance(10);
     ...
     */
    public AxisAlignedBB getSelectedBoundingBoxFromPool(final World worldID,
                                                        final int x,
                                                        final int y,
                                                        final int z)
    {
     ...
     */
    @Override
    public AxisAlignedBB getCollisionBoundingBoxFromPool(final World worldID,
                                                         final int x,
                                                         final int y,
                                                         final int z)
    {
     ...
     */
    @Override
    public void onBlockClicked(final World worldID,
                               final int x,
                               final int y,
                               final int z,
                               final EntityPlayer clickEntityPlayer)
    {}

    public boolean syncNeighbor(final World worldID,
                                final int x,
                                final int y,
                                final int z,
                                final int blockID,
     ...
     */
    @Override
    public boolean onBlockActivated(final World worldID,
                                    final int x,
                                    final int y,
                                    final int z,
                                    final EntityPlayer clickEntityPlayer,
    ...
    public void onPoweredBlockChange(final World worldID,
                                     final int x,
                                     final int y,
                                     final int z,
                                     final boolean bool)
     ...
     */
    @Override
    public void onNeighborBlockChange(final World worldID,
                                      final int x,
                                      final int y,
                                      final int z,
                                      final int neighborblockID)
    {
        if (!worldID.isRemote)
        {
            ...
            {
                onPoweredBlockChange(worldID, x, y, z, flag);
            }
        }
    }
     ...
     */
    @Override
    public MovingObjectPosition collisionRayTrace(final World worldID,
                                                  final int par2,
                                                  final int par3,
                                                  final int par4,
                                                  final Vec3 par5Vec3,
    ...
    /**
     * Called when a block is placed using its ItemBlock. Args: World, X, Y, Z, side, hitX, hitY, hitZ, block
     * metadata
     */
    @Override
    public int onBlockPlaced(final World worldID,
                             final int x,
                             final int y,
                             final int z,
                             final int side,
